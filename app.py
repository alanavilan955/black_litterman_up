...
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))  # asegura que bl_core.py se encuentre
import bl_core as core
...
with st.sidebar:
    ...
    if fuente == "Ejemplo generado":
        ...
        hist_long, lines_raw = core.generate_example(n_lines, n_months, seed, brk)
        sig = f"ej-{n_lines}-{n_months}-{seed}-{brk}"
    else:
        t_hist, t_lines = core.templates()
...
try:
    wide, data_warns = core.to_wide(hist_long)
    lines_raw = core.validate_lines(lines_raw)
...
flags = core.check_standard_breaks(wide)
...
with tab_d:
    ...
    cap = core.capacity_ton(lines_df)
...
with st.sidebar:
    ...
    try:
        sigma, cov_used, shrink = core.estimate_covariance(wide.values, cov_method)
    except ValueError as e:
        st.error(str(e))
        st.stop()
    T = len(wide)
    ...
    delta_auto = core.calibrate_delta(w0, sigma, target)
...
lo, hi, cap, issues = core.volume_bounds(lines_df, demand, w0, band, use_cap, use_min, use_band)
...
r = core.run(wide, w0, demand, views_df, tau, delta, sigma, lo, hi)
tab = core.allocation_table(r)
...
# ------------------------------------------------------------------ frontera
frontier = core.efficient_frontier(r["mu"], sigma, lo, hi, 30)
with tab_f:
    fig = go.Figure()
    if len(frontier):
...
with tab_s:
    if len(r["Q"]) == 0:
        ...
        sens = core.sensitivity(wide, w0, demand, views_df, tau, delta, sigma, lo, hi, pick)
...
with tab_c:
    text = core.commentary(r, views_df, cov_used, flags)
    ...
    xls = core.to_excel(r, views_df, lines_df, wide, frontier, sens, st.session_state.get("coment", text), params)
